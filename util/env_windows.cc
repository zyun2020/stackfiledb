...
class WindowsRandomAccessFile : public RandomAccessFile {
 public:
  ...
  bool IsMmap() const override { return false; }
 private:
  const ScopedHandle handle_;
  const std::string filename_;
};
...
class WindowsMmapReadableFile : public RandomAccessFile {
 public:
  ...
  bool IsMmap() const override { return true; }
...
class WindowsEnv : public Env {
 public:
  ...
  Status NewRandomAccessFile(const std::string& filename,
                             RandomAccessFile** result, bool use_mmap = true) override {
    *result = nullptr;
    DWORD desired_access = GENERIC_READ;
    ...
    if (!use_mmap || !mmap_limiter_.Acquire()) {
  ...
  Status NewWritableFile(const std::string& filename,
    ...
    DWORD share_mode = FILE_SHARE_READ;// 0;  // Exclusive access.
    ScopedHandle handle = ::CreateFileA(
        filename.c_str(), desired_access, share_mode,
  ...
  Status TruncateFile(const std::string& filename, uint64_t size) override {
      DWORD desired_access = GENERIC_WRITE;
      ...
      if (!::SetFilePointerEx(handle.get(), distance, nullptr, FILE_BEGIN)) {
          return WindowsError(filename, ::GetLastError());
      }
